...
class HexChassisApi:
    ...
    def _joint_cmd_callback(self, msg):
        try:
            ...
            # msg的数据类型为array.array，类型不匹配，因此需要增加校验
            velList = [float(v) if v is not None else 0.0
                        for v in list(msg.velocity)]
            if chassis is not None:
                chassis.start()
                chassis.motor_command(CommandType.SPEED, velList)
        except Exception as e:
            print(f"Error in joint_cmd_callback:\n {traceback.format_exc()}")
